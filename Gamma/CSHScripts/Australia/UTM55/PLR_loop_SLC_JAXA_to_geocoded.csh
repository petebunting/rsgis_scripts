...
foreach dir_name ($list_dir)
  ...
  #
  # Assign the file with the VV level 1.1 data to a variable.
  set image_name_vv_jaxa  = `ls IMG-VV*`
  #
  ...
  par_EORC_PALSAR $leader_name_jaxa temp.vv.slc.par $image_name_vv_jaxa temp.vv.slc
  #
  ...
  radcal_SLC temp.vv.slc temp.vv.slc.par $dir_name.vv.slc $dir_name.vv.slc.par 1 - 0 0 1 0 -115.0
  #
  ...
  set image_name_vv     = `ls al*.vv.slc`
  set image_name_vv_par = `ls al*.vv.slc.par`
  #
  ...
  multi_look $image_name_vv $image_name_vv_par $dir_name.vv.mli $dir_name.vv.mli.par $mltlook_rg $mltlook_az
  #
 ...
 offset_fitm $region"_offs_hh" $region"_snr_hh" $dir_name.diff_par $region"_coffs_hh" $region"_coffsets_hh" - 3 >> $dir_name.geocode_error.txt
  ...
  geocode_back $dir_name.vv.mli "$rs1" $region.geo_to_rdc $dir_name.vv.utm "$w1"
  #
  ...
  raspwr $dir_name.vv.utm "$w1" - - 10 10
  #

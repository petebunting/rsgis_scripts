...
print '''Script to process PLR ALOS scenes
python BatchGammaPLR.py <inDIR> <inSceneList> <processDIR> <outDIR> <zone>
 Input parameters:
 1) inDIR: directory containing raw, gzipped, data
 2) inSceneList: text file containing scenes to be processed, type '-' to process all scenes in inDIR
 3) processDIR: directory to process files
 ...
 5) zone + north or south e.g. 55n (enter LatLong or LL for lat long)
 6) parameter file - required if UTM zone is not recognised or to use non standard parameters'''
...
topoCorrectionTemplate = scriptsPath + 'CSHTemplates/General/PLR_topo_correction_template.csh'
...
generateHalphaTemplate = scriptsPath + 'CSHTemplates/PolSARPro/generate_Halpha.csh'
geoHalphaTemplate = scriptsPath + 'CSHTemplates/PolSARPro/geocode_HAalpha.csh'

if zone == '54N' or zone == '54S' or zone == '54':
    rawSLCTemplate = scriptsPath + 'CSHTemplates/Australia/PLR_raw_to_SLC_Australia_template.csh'
    jaxaSLCTemplate = scriptsPath + 'CSHTemplates/Australia/PLR_JAXA_1_1_to_SLC_Australia_template.csh'
    slcGeoTemplate = scriptsPath + 'CSHTemplates/Australia/PLR_SLC_to_GEO_Australia_template.csh'
    ...
    rawSLCTemplate = scriptsPath + 'CSHTemplates/General/PLR_raw_to_SLC_template.csh'
    jaxaSLCTemplate = scriptsPath + 'CSHTemplates/Australia/PLR_JAXA_1_1_to_SLC_Australia_template.csh'
    slcGeoTemplate = scriptsPath + 'CSHTemplates/General/PLR_SLC_to_GEO_withPAN_template.csh'
    res = 90
    ...
    llProjFile = scriptsPath + 'Projection/LL_WGS84.wkt'
    utmProjFile = scriptsPath + 'Projection/utm_z56.wkt'
    rawSLCTemplate = scriptsPath + 'CSHTemplates/SouthAmerica/PLR_raw_to_SLC_belize_template.csh'
    jaxaSLCTemplate = scriptsPath + 'CSHTemplates/Australia/PLR_JAXA_1_1_to_SLC_Australia_template.csh'
    slcGeoTemplate = scriptsPath + 'CSHTemplates/SouthAmerica/PLR_SLC_to_GEO_belize_template.csh'
    zone='LatLong'
    res = 90
    ...
    rawSLCTemplate = scriptsPath + 'CSHTemplates/UK/PLR_raw_to_SLC_UK_template.csh'
    slcGeoTemplate = scriptsPath + 'CSHTemplates/UK/PLR_SLC_to_GEO_UK_template.csh'
    subsetPAN = False
    zone='BNG'
    res = 5
                
elif zone == 'LL' or zone == 'LatLong':
    ...
    if numArgs != 7:
        print 'Zone not recognised - need to supply parameter file'
        exit()
    else:
        parametersListFile = sys.argv[6].strip()
        parametersList = open(parametersFile, 'rU')
        for parameter in parametersList:
            elements = parameter.split('=', parameter.count('='))
            ...
            elif elements[0].strip() == 'srtm':
                srtm = elements[1].strip()
            elif elements[0].strip() == 'pan':
                pan = elements[1].strip()  
            elif elements[0].strip() == 'llProjFile':
                llProjFile = elements[1].strip() 
            elif elements[0].strip() == 'utmProjFile':
                utmProjFile = elements[1].strip()
            elif elements[0].strip() == 'DEMResolution':
                res = elements[1].strip()   
            elif elements[0].strip() == 'SubsetDEM':
                subsetDEM = elements[1].strip()  
            elif elements[0].strip() == 'SubsetPAN':
                subsetPAN = elements[1].strip()
            elif elements[0].strip() == 'GenerateAlphaEntropy':
                genAlphaEntropy = elements[1].strip()            
...
if inSceneListFile == '-': # If no process list provided process all scenes in DIR
    ...
    inSceneList = open(inSceneListFile, 'rU')
...
for scene in processScenes:
    ...
    print '###########################################################'
    lev1_0 = True # Processing level of input data
    ...
    if LEDFile.find("1.0") == -1: # Check for 1.0 in filename, if not found assume 1.1 data
        print "\tInput data is level 1.1, converting to GAMMA SLC then georefferencing"
        lev1_0 = False
        
        replaceLEDCMD = 'sed \'s/LEDFILENAME/' + LEDFile + '/g\' ' + jaxaSLCTemplate + ' > ' + temp1
    ...
    # Replace Scenename
    replaceSceneNameSLCCMD = 'sed \'s/SCENENAME/' + scene + '/g\' ' + temp4 + ' > ' + rawSLCScript
    ...
    if outDIR != '-':
        ...
        for fileName in fileList:
            if lev1_0:
                ...
                newFileName = fileName.replace('lev1_1','lev1_5')
        ...
        meta = BatchGammaMeta(newSceneDIR, "PLR")
        meta.writeMetaText()

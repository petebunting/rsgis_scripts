    ...
    def generateENVIHeaderSpatial(self, filelist):
        try:
            for filename in filelist:
                ...
                outputFile.write( 'sensor type = Unknown\n')
                outputFile.write( 'byte order = 1\n')
                outputFile.write( "map info = {%s, 1.500, 1.500, %s, %s, %e, %e, %s, South, %s, units=meters}\n" % (str(parameters[8]), str(parameters[4]), str(parameters[3]), float(parameters[6]), float(parameters[6]), str(parameters[9]), str(parameters[7])))
                outputFile.write( 'wavelength units = Unknown\n')

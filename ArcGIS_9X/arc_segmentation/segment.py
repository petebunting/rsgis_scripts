    ...
    def performSegmentation(self, segmentLocation, sigma, k, min, inputDIR, outputDIR, tmpDIR, filename, printOut):
        ...
        if len(filenameSplit) == 2:
            if filenameSplit[1] == 'jpg':
                ...
                printOut.printMessage('Image Type: '+imageType)
                ...
                if not imageType == 'ppm':
                    ...
                    os.chdir(tmpDIR)
                    im.save('tmp.ppm', 'ppm')
                    imageFile = tmpDIR + 'tmp.ppm'
                command = segmentLocation + 'segment ' + str(sigma) + ' ' + str(k) + ' ' + str(min) + ' ' + imageFile + ' ' + tmpDIR+'tmp_output.ppm'
                ...
                im = Image.open(tmpDIR+'tmp_output.ppm')
                im.save(outFileName)
                os.remove(tmpDIR+'tmp_output.ppm')
                os.remove(tmpDIR+'tmp.ppm')

        
    def segmentDir(self, segmentLocation, sigma, k, min, inputDIR, outputDIR, tmpDIR, printOut):
        os.chdir(inputDIR)
        cwd = os.getcwd()
        printOut.printMessage('Working directory: '+cwd)
        fileList = os.listdir(inputDIR)
        printOut.printMessage(fileList)
        for filename in fileList:
            printOut.printMessage('filename: '+filename)
            self.performSegmentation(segmentLocation, sigma, k, min, inputDIR, outputDIR, tmpDIR, filename, printOut)
    
    def printMessage(self, outText):
    	print outText
    	
    def run(self):
        usrSegmentDIR = "/Users/pete/Desktop/segment/"
        usrSigma = "0.5"
        usrK = "500"
        usrMin = "20"
        usrInputDIR = "/Users/pete/Desktop/images/"
        usrOutputDIR = "/Users/pete/Desktop/images/output/"
        tmpOutputDIR = "/Users/pete/Desktop/images/tmp/"

...
class NameJERSoutParFile (object) :

    def getSceneName(self, parFilename, outFilename):
        ...
        sceneParameters = sceneName + ',' + orderNo + ',' + jaxaSceneName + ',' + processingLevel + ',' + pathRow + ',' + obsdate + ',' + centerLat + ',' + centerLong + '\n'
        
        outFile.write(sceneParameters)
    ...
    def tarFile(self, inDIR, sceneName):
        #tarcmd = 'tar -czf ' +  inDIR + '/' + sceneName + '.tar.gz  ' + sceneName
        #delcmd = 'rm -R ' + inDIR + '/' + sceneName
        tarcmd = 'tar -czf ' + sceneName + '.tar.gz  ' + sceneName + '/'
        delcmd = 'rm -R ' + inDIR + '/' + sceneName
        os.chdir(inDIR)
        os.system(tarcmd)
        os.system(delcmd)
    
    def run(self, inDIR, folderName, outText):
        sceneLABfile = self.getHeaderFileName(inDIR, folderName)
        sceneName = self.getSceneName(sceneLABfile, outText)
...
if __name__ == '__main__':
    ...
    outText = sys.argv[3].strip()
    obj = NameJERS()
    obj.run(inDIR, folderName, outText)

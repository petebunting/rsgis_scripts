...
class ALOSUnpackRenameTar (object):
    ...
    def findALOSParams(self, dataList, productID):
        for data in dataList:
            if data.sameProdID(productID):
                return data
        raise Exception('Could not find productID..')
    ...
    def untarRename(self, fileList, alosParams, ext, outDIR, outDIRTar):
        for file in fileList:
            try:
                print file
                productID = self.getALOSSceneID(file)
                print productID
                ALOSParam = self.findALOSParams(alosParams, productID)
